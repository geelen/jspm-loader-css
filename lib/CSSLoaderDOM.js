...
export class CSSLoaderDOM extends CSSModuleLoaderProcess {
  ...
  _sortAndInjectInDom (styleSheet) {
    ...
    const isExistingStyle = (depName) => Boolean(this._styleMeta.get(depName))
    const isValidStyleMeta = (depName) => Boolean(this._styleMeta.get(depName)[USE_STYLE_TAGS ? 'injectableSource' : 'blobUrl'])

    this._jspmCssModulesContainer.innerHTML = sortedDependencies
      .filter((depName) => isExistingStyle(depName) && isValidStyleMeta(depName))
      .map(injectionFunc)
      .join('')
