...
// tag if not.
  ...
  bundle( loads, opts ) {
    let css = this._cache._source.join( "\n" )
      ...
      .replace( /[\u2028]/g, "\\u2028" )
      .replace( /[\u2029]/g, "\\u2029" );
    return `(function(c){var d=document,a='appendChild',i='styleSheet',s=d.createElement('style');var preload=d.querySelector('#preloaded-styles');if (preload) preload.remove();s.type='text/css';d.getElementsByTagName('head')[0][a](s);s[i]?s[i].cssText=c:s[a](d.createTextNode(c));})("${css}");`
  }
}
